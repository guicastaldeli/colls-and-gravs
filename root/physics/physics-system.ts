...
export class PhysicsSystem {
    private gravity: number = 80.0;
    public angularDamping: number = 1.0;
    ...
    private checkStability(obj: PhysicsObject): void {
        ...
        if(isStable) {
            ...
            if(vec3.length(obj.angularVelocity) < 1.0) {
                ...
                const randomTorque = vec3.fromValues(
                    (Math.random() - 0.5) * 0.5,
                    0.0,
                    (Math.random() - 0.5) * 0.5
                );
    ...
    private checkEdgeStability(obj: PhysicsObject, groundLevel: number): void {
        if(obj.isStatic && vec3.length(obj.velocity) < 0.1) return;
        ...
        if(!this.isPointInConvexHull(comGround, hullPoints)) {
            ...
            const supportCenter = vec3.create();
            for(const p of hullPoints) vec3.add(supportCenter, supportCenter, p);
    ...
    private stabilityCom(obj: PhysicsObject, groundLevel: number): boolean {
        ...
        const isStable = (
            comGroundX >= minX && comGroundX <= maxX &&
    ...
    private applyGravityTorque(obj: PhysicsObject, deltaTime: number): void {
        if(obj.isStatic || !obj.isStable) return;
    ...
    private calculateConvexHull(points: vec3): vec3[] {
        ...
        do {
            ...
            for(let i = 0; i < points.length; i++) {
                ...
                const cross = 
                (points[q][0] - points[p][0]) * (points[i][2] - points[q][2]) -
    ...
    private isPointInConvexHull(point: vec3, hull: vec3[]): boolean {
    ...
    private fixedUpdate(deltaTime: number): void {
        ...
        for(const obj of this.physicsObjects) {
            ...
            if(!isStable && obj.isStable) this.applyGravityTorque(obj, deltaTime);
            if(!obj.isOnGround) vec3.scale(obj.angularVelocity, obj.angularVelocity, this.angularDamping);
